...
#OPEN PANTRY ACCESS
@app.route('/updatepant')
def update_pantry():
	with sql.connect('recipebase.db') as con:
		cur = con.cursor()
		print("attepmting to update pantry")
		msg = "Unable to update pantry item, please try again"
		try:
			name = request.form['Name']
			delete = request.form['deleteflag']
			if not delete:
				newamount = request.form['new_amount']
				cur.execute("Update Pantry SET Pantry.Quantity = ? WHERE Pantry.Name = ?",[newamount],[name])
				msg = "Updated item in Pantry"
				con.commit()	
			else:
				cur.execute("DELETE FROM Pantry WHERE Pantry.Name = ?",[name])
...
def listpant(msg=""):
	...
	return render_template('listpant.html', rows = rows, msg = msg)
...
#Add landing page for when pantry is empty, suggest Adding to the pantry
#OPEN PANTRY ACCESS
@app.route('/find')
			...
			try:
				...
				if flag != -1:		
					...
					if currentids:
						...
						id = 1
					cur.execute("INSERT INTO Recipe (RecipeID,Name,NumSteps,NumIngredients) VALUES (?,?,?,?)", (id,name.capitalize(), 0, 0))
			...
			try:
				...
				if not name and Steps != "2":
					...
					if Steps=="1": 
						...
						cur.execute("INSERT INTO Ingredients (RecipeID, IngredientValue, Measurement, Ingredient) VALUES (?,?,?,?)", (rID,quantity,measurement,name.capitalize() ))
...
def addpant():
	...
	if request.method == 'POST':
		with sql.connect("recipebase.db") as con:
				...
				else: #Pantry item already exists, try and update its quantity
					cur.execute("Select Pantry.Quantity, Pantry.Measurement from Pantry WHERE Pantry.Name = ?",[Name])
					existing = cur.fetchall()
					if existing[0][1] == Measurement:
						amount = existing[0][0] + Quantity
						cur.execute("UPDATE Pantry SET Pantry.Quantity = ? WHERE Pantry.Name = ?", [amount],[Name])	
						con.commit()
					else:
						#implement some conversions?
						msg = "existing pantry item exists, please choose matching measurement"
				rows = getpant()
